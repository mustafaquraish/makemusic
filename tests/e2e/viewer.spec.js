...
test.describe('Minimap', () => {
  ...
  test('clicking minimap navigates', async ({ page }) => {
    ...
    // Click near the bottom of the minimap (start of song, which is a high scroll position)
...
test.describe('Theme Toggle', () => {
  ...
  test('theme icon changes', async ({ page }) => {
    await loadViewer(page);
    const iconBefore = await page.locator('#theme-icon').textContent();

    await page.keyboard.press('t');
    await page.waitForTimeout(200);

    const iconAfter = await page.locator('#theme-icon').textContent();
...
test.describe('Edit Mode', () => {
  ...
  test('clicking empty space in edit mode adds a new note', async ({ page }) => {
    ...
    const container = page.locator('#piano-roll-container');
    const box = await container.boundingBox();
    // Click far-left where there should be no notes (key_index 0-5 range)
...
test.describe('Edge Cases', () => {
  ...
  test('nextNoteId does not collide with existing note IDs >= 10000', async ({ page }) => {
    ...
    // Enter edit mode and add a note
    await page.keyboard.press('e');
    await page.keyboard.press('Home');
    await page.waitForTimeout(300);
    const container = page.locator('#piano-roll-container');
...
test.describe('Click-and-Drag Note Creation', () => {
  test('simple click creates default-size note', async ({ page }) => {
    await loadViewer(page);
    await page.keyboard.press('e');
    await page.waitForTimeout(200);
    await page.keyboard.press('Home');
    ...
    const newNote = page.locator('.note-block.selected');
    const dur = await newNote.getAttribute('data-duration');
    expect(parseFloat(dur)).toBeCloseTo(1.0, 0);
  });
...
test.describe('Right-Click Context Menu', () => {
  ...
  test('context menu has expected items', async ({ page }) => {
    ...
    expect(count).toBe(3); // Toggle Hand, Duplicate, Delete
  });
